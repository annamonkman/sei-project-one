...
The game has been deployed with GitHub Pages and is available here, this currently has some issues with images showing up that I will sort out asap. 
OR 
1.	Access the source code from the ‘Clone or download’ button
2.	Open the index.html file in your browser to start the game
Click on start button to start game, navigate the snail with the arrow buttons, get to the pipes at the end to win but avoid the rats, holes and fire. Green is safe land & go on the boards to get across the stream. 
...
I wanted to give the game my own twist so chose to feature a snail instead of a frog, with the goal to help the snail get across a post-apocalyptic-esque wasteland with rats instead of cars and crisp packets and twigs for the boards. 
...
I then added a snail class to the cell at the snail start position. When adding classes for each item I gave these a background colour in CSS so it was made clear visually where they were positioned. To allow the user to move the snail I added an event listener on the keyup event and a handleKeyUp function, and addSnail and removeSnail functions. 
The handleKeyUpfunction 
-	called the removeSnail function, to remove the snail (class) from its current position 
-	change the snailcurrentposition based on the arrow key that was pressed, with if statements to make sure the functionality wouldn’t work beyond the grid barriers. 
-	called the addSnail function to add the snail (class) back to its current position. 
...
Initially I had a ‘safeLandClass’ but then that became the objects themselves (ie. ‘roadClass’) as I progressed. 
For the areas that took up a lot of space, I stored an array of the cell index numbers of where I wanted the areas to be placed in a const. ie. const unsafeLandArray = [0, 1, 2, 3, 4 etc.].
To add the classes to their respective cells, I created functions ie. addWaterClass(position) so, for each item add the water class to the cell. Then I called the functions towards the end of the coding, passing the array as an argument. 
...
I started with animating the rats. I tried out one first and stored the Timer in a let variable.  
...
### Die/Lose Logic
I needed to create a function for what happens when the snail goes on unsafe land. When called the function snailDie . runs an if statement that checks if the cells with index of the snailCurrentPosition contains unsafeLandClass, then that sets a Timeout to call the removeSnail() function. 

In HTML I created divs for the pop-ups that would appear at the start of the game to start and when the player loses or wins.  So within the timeOut within the snailDie function I wrote code so that the popupLose would display as ‘block’ and the player can press the restart button to restart the game. 
...
### Styling
I looked at my archive of images for some inspiration. I had been to an exhibition of .. work at Sadie Coles. I hoped to give the arcade game a more contemporary feel. A washy painterly yet digital . haziness. Colour scheme. Dreamy . 
I drew the visuals in Procreate on an iPad at a very low resolutions since they’d be displayed very small. 
All of the visuals are background images or colours that were applied to the cells. 
There are some things details I would adjust in the styling, such as aligning the pipes, changing the png of the snail when it moves in different directions, adding more custom styling to the popups. Working out how to make the fire GIF move. I aldo planned to have some ‘trash’ in the main browser window. 
